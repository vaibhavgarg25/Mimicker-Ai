...
export default function DashboardPage() {
  ...
  const steps = [
    { number: 1, title: "Upload Video", description: "Record and upload your screen" },
    { number: 2, title: "Analysis", description: "Extract actions from video" },
    { number: 3, title: "Credentials", description: "Add login details (optional)" },
    { number: 4, title: "Generate Script", description: "Create automation script" },
  ];
  ...
  const updateCredential = (id: string, field: keyof Omit<Credential, 'id'>, value: string) => {
  ...
  const goToStep = (step: number) => {
    if (step <= currentStep || (step === 2 && selectedFile) || (step === 3 && analysisResult)) {
      setCurrentStep(step);
    }
  };
  ...
  const canProceedToNext = () => {
    switch (currentStep) {
      ...
      case 2: return analysisResult !== null;
      case 3: return true; // Credentials are optional
      case 4: return false;
            ...
            <CardHeader>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-base sm:text-lg">Upload a video</CardTitle>
                    <CardDescription className="text-xs sm:text-sm">Capture a screen recording — recommended 30s–3m</CardDescription>
                  </div>
                </div>
          ...
          <Card className={`${cardBase} border-primary/12`}>
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <CardTitle className="text-base sm:text-lg">Analysis Status</CardTitle>
              </div>
            </CardHeader>
            <CardContent>
              {!analysisResult ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <h4 className="font-semibold text-sm sm:text-base">Detected Actions</h4>
                    <ul className="space-y-2">
                      {analysisResult.detectedActions?.map((action: string, idx: number) => (
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-base sm:text-lg">Credentials (Optional)</CardTitle>
                    <CardDescription className="text-xs sm:text-sm">Add login details for automation</CardDescription>
                  </div>
                </div>
                <Button 
                  variant="outline" 
              ...
              {credentials.length === 0 ? (
                <div className="py-6 text-center">
                  ...
                  <p className="text-sm text-muted-foreground mb-4">
                    No credentials added. You can skip this step or add login details for automated form filling.
                  </p>
                ...
                <div className="space-y-4">
                  {credentials.map((credential) => (
                    <div key={credential.id} className="p-4 rounded-lg border border-white/10 bg-white/2 space-y-3">
                      <div className="flex items-center justify-between">
                        <Input
                          placeholder="Credential Label (e.g., Gmail, GitHub)"
                          value={credential.label}
                          onChange={(e) => updateCredential(credential.id, 'label', e.target.value)}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div>
                          <Label htmlFor={`username-${credential.id}`} className="text-xs text-muted-foreground">Username/Email</Label>
                          <Input
                        ...
                        <div>
                          <Label htmlFor={`password-${credential.id}`} className="text-xs text-muted-foreground">Password</Label>
                          <Input
                      ...
                      <div>
                        <Label htmlFor={`apikey-${credential.id}`} className="text-xs text-muted-foreground">API Key (if needed)</Label>
                        <Input
                          id={`apikey-${credential.id}`}
                          placeholder="sk-..."
                          value={credential.apiKey}
                          onChange={(e) => updateCredential(credential.id, 'apiKey', e.target.value)}
                          className="bg-transparent border-white/10 text-sm mt-1"
                        />
                      </div>
                      
                      <div>
                        <Label htmlFor={`notes-${credential.id}`} className="text-xs text-muted-foreground">Notes</Label>
                        <Textarea
          ...
          <Card className={`${cardBase} border-primary/12`}>
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <CardTitle className="text-base sm:text-lg">Generate Script</CardTitle>
              </div>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="p-4 rounded-lg bg-white/5 border border-white/10">
                  <h4 className="font-medium mb-2">Ready to generate automation script</h4>
                  <ul className="space-y-1 text-sm text-muted-foreground">
                    <li className="flex items-center gap-2">
                      <Check className="size-4 text-green-400" />
                      Video analyzed: {analysisResult?.detectedActions?.length || 0} actions found
                    </li>
                    <li className="flex items-center gap-2">
                      <Check className="size-4 text-green-400" />
                ...
                <Button 
                  ...
                  className="w-full"
                >
                  {isRunningScript ? "Generating Script..." : "Generate & Run Script"}
                </Button>
      ...
      <div className="max-w-6xl mx-auto mt-10 px-4 sm:px-6 py-8 sm:py-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 sm:mb-10">
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold">Welcome back, {userData.name}</h1>
              <p className="text-xs sm:text-sm text-muted-foreground">Member since {userData.joinDate}</p>
            </div>
          </div>

          <div className="flex items-center gap-2 sm:gap-3">
            <Button variant="ghost" size="sm" className="text-xs sm:text-sm" onClick={() => window.location.reload()}>
              Refresh
            </Button>
          </div>
        </div>
            ...
            {currentStep === 1 && (
              <Card className={`${cardBase} border-primary/12`}>
                <CardHeader>
                  ...
                  <CardDescription className="text-xs sm:text-sm">Latest videos and processing status</CardDescription>
                </CardHeader>
                <CardContent>
                  {uploadedVideos.length === 0 ? (
                    <div className="py-6 sm:py-8 text-center text-muted-foreground">
                      <p className="text-sm sm:text-base">No videos uploaded yet.</p>
                      <p className="mt-2 text-xs sm:text-sm">Upload a recording to start automating.</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {uploadedVideos.slice(0, 3).map((video, idx) => (
                        <div key={idx} className="flex items-center justify-between gap-2 sm:gap-4 p-2 sm:p-3 rounded-lg bg-white/3">
                          <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                            {video.thumbnail ? (
                              <img src={video.thumbnail} alt="thumb" className="w-10 h-7 sm:w-14 sm:h-10 object-cover rounded-md flex-shrink-0" />
                            ) : (
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="font-medium text-xs sm:text-sm truncate">{video.original_name || "Untitled"}</div>
                              <div className="text-xs text-muted-foreground mt-0.5 sm:mt-1">
                                <span className="block sm:inline">
                                  {(video.upload_timestamp && new Date(video.upload_timestamp).toLocaleDateString()) || "—"}
                                </span>
                                <span className="hidden sm:inline"> • </span>
                                <span className="block sm:inline">{video.duration || "—"}</span>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-1 sm:gap-3 flex-shrink-0">
                            ...
                            <Button variant="ghost" size="sm" className="p-1 sm:p-2" onClick={() => toast("Open player (not wired)")}>
            ...
            <div className="space-y-4 sm:space-y-6">
                ...
                <CardContent>
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="font-medium text-sm sm:text-base truncate">{userData.name}</div>
                        <div className="text-xs sm:text-sm text-muted-foreground truncate">{userData.email}</div>
                      </div>
                    </div>
                    ...
                    <div className="text-center flex-shrink-0">
                      <div className="w-16 h-16 sm:w-20 sm:h-20">
                        <svg className="w-full h-full" viewBox="0 0 80 80">
                          ...
                          <text x="40" y="45" textAnchor="middle" fill="#fff" fontSize="12" fontWeight="600" className="sm:text-sm">{userData.successRate}</text>
                        </svg>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:gap-3 text-center mt-4">
                    <div>
                      ...
                      <div className="font-bold text-sm sm:text-lg text-primary">{userData.scriptsGenerated}</div>
                    </div>
                    <div>
                      <div className="text-xs text-muted-foreground">Upload time</div>
                      <div className="font-bold text-sm sm:text-lg text-primary">{userData.totalUploadTime}</div>
                    </div>
                    <div>
                      <div className="text-xs text-muted-foreground">Joined</div>
                      <div className="font-bold text-sm sm:text-lg">{userData.joinDate.split('/').slice(0,2).join('/')}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className={`${cardBase} border-primary/12`}>
                ...
                <CardContent className="space-y-2 sm:space-y-3">
                  <Button variant="outline" className="w-full justify-start gap-2 sm:gap-3 bg-transparent text-xs sm:text-sm" onClick={() => toast("Opening analytics...")}>
                    <BarChart3 className="w-3 h-3 sm:w-4 sm:h-4" />
                    View Analytics
                  </Button>
                  <Button variant="outline" className="w-full justify-start gap-2 sm:gap-3 bg-transparent text-xs sm:text-sm" onClick={() => toast("Opening settings...")}>
                    <Settings className="w-3 h-3 sm:w-4 sm:h-4" />
                    Account Settings
                  </Button>
                  <Button variant="ghost" className="w-full justify-start gap-2 sm:gap-3 text-xs sm:text-sm" onClick={() => toast("Invite flow coming soon")}>
              ...
              <Card className={`${cardBase} border-primary/12`}>
                <CardHeader>
                  <CardTitle className="text-base sm:text-lg">Current Progress</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-sm">Step {currentStep} of 4</span>
                      <span className="text-xs text-muted-foreground">{Math.round((currentStep / 4) * 100)}%</span>
                    </div>
